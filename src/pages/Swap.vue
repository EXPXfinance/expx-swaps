...
import { SOR } from '@expx/sor';
import { Swap, Pool } from '@expx/sor/dist/types';

import config from '@/config';
import provider from '@/utils/provider';
import { BSC_KEY, scale, isAddress, getBscScanLink } from '@/utils/helpers';
...
const GAS_PRICE = process.env.APP_GAS_PRICE || '10000000000';
const MAX_POOLS = 4;
...
export default defineComponent({
    ...
    setup() {
        ...
        const assetOutSymbol = computed(() => {
            let address = assetOutAddressInput.value === BSC_KEY ?
                config.addresses.wbnb : assetOutAddressInput.value;

            return store.getters['assets/metadata'][address].symbol
        });
        ...
        watch(assetInAddressInput, () => {
            Storage.saveInputAsset(assetInAddressInput.value);
            onAmountChange(activeInput.value);
        });

        watch(assetOutAddressInput, async () => {
            Storage.saveOutputAsset(assetOutAddressInput.value);
            if (sor) {
                await sor.setCostOutputToken(assetOutSymbol.value, config.backendUrl);
            }
            onAmountChange(activeInput.value);
        });
        ...
        async function initSor(): Promise<void> {
            sor = new SOR(
                ...
                MAX_POOLS,
                `${config.backendUrl}/subgraph/pools`,
                config.addresses.poolsInfo
            );

            const assetInAddress = assetInAddressInput.value === BSC_KEY
                ? config.addresses.wbnb
                : assetInAddressInput.value;
            const assetOutAddress = assetOutAddressInput.value === BSC_KEY
                ? config.addresses.wbnb
        ...
        async function handleTransaction(transaction: any, text: string): Promise<void> {
            ...
            const type = transactionReceipt.status === 1
                ? 'success'
                : 'error';
            const link = getBscScanLink(transactionReceipt.transactionHash);
            store.dispatch('ui/notify', {
                text,
                type,
                link,
        ...
        function isWrapPair(assetIn: string, assetOut: string): boolean {
            if (assetIn === BSC_KEY && assetOut === config.addresses.wbnb) {
                return true;
            }
            if (assetOut === BSC_KEY && assetIn === config.addresses.wbnb) {
                return true;
            }
            return false;
        }
